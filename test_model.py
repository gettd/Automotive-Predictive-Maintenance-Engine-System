...
def create_sequences(df, sequence_length, feature_cols, target_col):
    """Create overlapping sequences without predictive horizon shift"""
    X, y = [], []
    ...
    for i in range(len(df) - sequence_length):
        ...
        seq_y = labels[i + sequence_length - 1]  # label at current time
        X.append(seq_x)
        y.append(seq_y)
...
def first_crossing(signal, timestamps, threshold=0.5):
...
for i, file in enumerate(selected_files, 1):
    ...
    # Analysis of first crossing (earliness)
    ...
    if predicted_cross is not None and true_cross is not None:
        print(f"Earliness (prediction - true): {predicted_cross - true_cross} sec")
    print("=================================\n")
